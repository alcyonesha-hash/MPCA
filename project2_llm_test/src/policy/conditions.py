...
class ConditionC0:
    """
    Baseline: Opportunity에서만 응답

    Spec 6.3 C0:
    - is_opportunity인 경우에만 응답 후보
    - 응답 확률은 config로 조정 가능
    ...
    def should_respond(
        ...
        # 확률 기반 응답
        if random.random() < self.config.baseline_respond_prob:
            # C0: 고정 delay (baseline)
            return True, self.config.default_delay, f"Baseline: opportunity ({opp_result.opp_type})", {'utt_type': 'baseline'}
        else:
            return False, 0.0, "Baseline: skipped by probability", {}


class ConditionC1:
    """
    +M1: 참여 확률 + relevance gate + timing

    Spec 6.3 C1:
    - p_mention, p_spont 사용
    ...
    def should_respond(
        ...
        if opp_result.opportunity_type == OpportunityType.DIRECT_MENTION:
            prob = self.config.p_mention
        else:
            prob = self.config.p_spont
        ...
        if random.random() < prob:
            ...
            reason = f"C1: {opp_result.opp_type}, p={prob:.2f}, rel={relevance:.2f}, type={utt_type}"
            return True, delay, reason, metadata
        else:
            return False, 0.0, f"C1: skipped by prob {prob:.2f}", {}
